  ...
  result, sIP = VH:GetUserIP(nick2kick)
    if string.len(sIP)>0 then
	 ...
	 if diff_time<vote_delay then
    	  SendMessageToUser(string.format(" икать слишком часто вредно :) подожди "..(vote_delay-diff_time).." секунд."), nick, botname)
	 else
	  ...
	  SendMessageToAll(string.format(msg.." пользовател€ "..nick2kick.." проголосовал(а) "..nick..". ¬сего голосов: "..i.." из "..max_votes.."."), botname)
	  
	  
	  if i >= max_votes then
	   result, iClass = VH:GetUserClass(nick2kick)
	   if iClass <6 then
	    VH:KickUser(botname, nick2kick, "“ак решило быдло.")
	   end
